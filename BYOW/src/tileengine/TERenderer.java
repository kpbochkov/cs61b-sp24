...
public class TERenderer {
    ...
    public void renderFrameTest(Board board) {
        StdDraw.clear(new Color(0, 0, 0));  // Clear the screen
        drawTiles(board.getGrid());  // Draw the grid with rooms and walls

        // Now render the room numbers (after tiles)
        for (Room room : board.getRooms()) {
            int textX = room.getX() + room.getWidth() / 2; // Center of the room
            int textY = room.getY() + room.getHeight() / 2; // Center of the room

            String roomNumber = Integer.toString(board.getRooms().indexOf(room) + 1);

            // Set text color and draw the room number in the middle of each room
            StdDraw.setPenColor(Color.WHITE);
            StdDraw.text(textX + 0.5, textY + 0.5, roomNumber);  // Adjust text position if necessary
        }

        StdDraw.show();  // Update the screen with the new drawing
    }
